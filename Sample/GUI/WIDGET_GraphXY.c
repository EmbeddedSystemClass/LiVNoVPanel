...
static GUI_POINT _aPoint[3][361];
...
static void _InitPoints(void) {
  ...
  for (i = 0; i <= 360; i++) {
    float a;
    a = i * DEG2RAD;
    _aPoint[0][i].x = (100 - r * cos(a));
    _aPoint[0][i].y = (100 + r * sin(a) * i / 360);
    _aPoint[1][i].x = (100 - r * cos(a));
    _aPoint[1][i].y = (100 + r * sin(a) * i / 300);
    _aPoint[2][i].x = (100 - r * cos(a) * 0.9);
    _aPoint[2][i].y = (100 + r * sin(a) * i / 320);
  }
}
