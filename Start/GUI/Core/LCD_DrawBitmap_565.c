...
static void _DrawBitmap(int x0, int y0, int xsize, int ysize, const U8 GUI_UNI_PTR * pPixel, const LCD_LOGPALETTE GUI_UNI_PTR * pLogPal, int xMag, int yMag) {
  ...
  if ((Mode == 565) && (SwapRB == _BitmapSwapRB) && (Mag == 1)) {
    UseDrawBitmap = 1;
  }
  #else
  if (GUI_Context.pDeviceAPI->pMemDevAPI) {
    if (((GUI_Context.hDevData && (GUI_Context.pDeviceAPI->pMemDevAPI->pfIndex2Color == LCD_Index2Color_565)) ||
        ((GUI_Context.hDevData == 0) && (Mode == 565) && (SwapRB == _BitmapSwapRB)))  && (Mag == 1)) {
      UseDrawBitmap = 1;
    }
  }
  #endif
    ...
    if (Mag == 1) {
      ...
      for (y = 0; y < ysize; y++) {
        ...
        for (x = 0; x < xsize; x++) {
          ...
          Index = *p++;
          if (Index != PrevIndex) {
            LCD_SetColor(_BitmapSwapRB ? LCD_Index2Color_M565(Index) : LCD_Index2Color_565(Index));
            PrevIndex = Index;
          }
